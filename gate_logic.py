...
import time
import RPi.GPIO as GPIO
from config import GATE_VERIFY_API_URL, GATE_ID, TEST_MODE
...
PWM_PIN = 17             # BCM 핀 번호
GATE_OPEN_ANGLE = 7.5    # 게이트 열림 위치 (DutyCycle)
GATE_CLOSE_ANGLE = 2.5   # 게이트 닫힘 위치 (DutyCycle)
GATE_FREQ = 50           # PWM 주파수 (50Hz)
GATE_OPEN_TIME = 3       # 게이트 열려있는 시간 (초)

def open_gate():
    """서보모터를 제어하여 게이트를 여닫는 함수"""
    GPIO.setmode(GPIO.BCM)
    GPIO.setup(PWM_PIN, GPIO.OUT)
    pwm = GPIO.PWM(PWM_PIN, GATE_FREQ)
    try:
        ...
        pwm.ChangeDutyCycle(GATE_OPEN_ANGLE)
        print(f"🚦 BCM {PWM_PIN} → {GATE_OPEN_ANGLE} DC (Gate OPEN)")
        time.sleep(GATE_OPEN_TIME)

        pwm.ChangeDutyCycle(GATE_CLOSE_ANGLE)
        print(f"🚪 BCM {PWM_PIN} → {GATE_CLOSE_ANGLE} DC (Gate CLOSE)")
        time.sleep(0.5)

    except Exception as e:
        print(f"🚨 PWM 제어 중 오류: {e}")
    finally:
        pwm.stop()
        GPIO.cleanup()
        print("✅ PWM 정지 및 GPIO 정리 완료.")

def display_status(is_success, reason=""):
    """게이트 UX 표시 (콘솔용)"""
    if is_success:
        print("✅ Green Light ON: Access Granted. Gate Opening...")
    else:
        print(f"❌ Red Light ON: Access Denied. Reason: {reason}")

# ======================================================
# --- B. Core Logic Function (Verification & Control) ---
# ======================================================

def verify_consume_token(token_string: str, gate_id: str = GATE_ID) -> dict:
    """
    QR 토큰을 받아 서버 검증 후 게이트를 여는 핵심 로직
    TEST_MODE=True일 경우 서버 요청을 건너뛰고 게이트를 직접 개방
    """
    print(f"\n--- Token Verification Started (Gate ID: {gate_id}) ---")

    # 1️⃣ TEST MODE일 경우: 서버 요청 생략
    if TEST_MODE:
        print("🧪 [TEST MODE ENABLED] 서버 검증을 생략합니다.")
        time.sleep(0.5)
        display_status(True)
        open_gate()
        return {"status": "OPEN", "reason": "TEST_MODE_OK"}

    # 2️⃣ JWT 파싱 (로컬 시그니처 검증은 비활성화)
    try:
        payload = jwt.decode(token_string, options={"verify_signature": False})
    except Exception as e:
        reason = f"TOKEN_PARSING_ERROR: {e.__class__.__name__}"
        print(f"🚨 Token 파싱 실패: {reason}")
        display_status(False, reason)
        return {"status": "BLOCK", "reason": reason}
    ...
    api_payload = {
        "token": token_string,
        "gate_id": gate_id,
        "device_hash": payload.get("device_hash", "UNKNOWN")
    }

    try:
        print(f"🌐 서버 검증 요청 중: {GATE_VERIFY_API_URL}")
        response = requests.post(GATE_VERIFY_API_URL, json=api_payload, headers=headers, timeout=0.4)
        ...
        if response.status_code == 200:
            ...
            print(f"🌐 서버 오류 발생: {reason}")
            display_status(False, reason)
            return {"status": "BLOCK", "reason": reason}

    except requests.exceptions.Timeout:
        reason = "TIMEOUT"
        print("🌐 서버 응답 시간 초과 (Default BLOCK)")
        display_status(False, reason)
        return {"status": "BLOCK", "reason": reason}

    except requests.exceptions.RequestException as e:
        reason = f"NETWORK_FAIL: {e.__class__.__name__}"
        print(f"🌐 네트워크 오류: {reason}")
        display_status(False, reason)
        return {"status": "BLOCK", "reason": reason}
